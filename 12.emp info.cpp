...
employee employeeDatabase::readEmp(int pos) {
    fstream data_file(data_file_name.c_str(), ios::binary | ios::in | ios::ate);
    employee emp;

    if (!data_file || pos == -1 || pos >= data_file.tellg()) {
        emp.empId = -1;
        return emp;
    }
...
employeeDatabase::employeeDatabase() {
    fstream data_file(data_file_name.c_str());
    fstream index_file(index_file_name.c_str());
    if (data_file.fail() || index_file.fail()) {
        cout << "Created a new file." << endl;
        data_file.open(data_file_name.c_str(), ios::binary | ios::out | ios::app);
        index_file.open(index_file_name.c_str(), ios::binary | ios::out | ios::app);
    } else {
...
void employeeDatabase::addEmployee(int eid, char name[50], char dest[50], int sal) {
    fstream data_file(data_file_name.c_str(), ios::binary | ios::out | ios::app);
    fstream index_file(index_file_name.c_str(), ios::binary | ios::in);
    ...
    if (update) {
        index_file.open(index_file_name.c_str(), ios::binary | ios::in | ios::out);
        index_file.seekp(ipos, ios::beg);
        index_file.write((char *)&current, sizeof(current));
    } else {
        bool written = false;
        vector<index_pair> index_pairs;
        index_file.open(index_file_name.c_str(), ios::binary | ios::in);
        ...
        index_file.clear();
        index_file.close();
        index_file.open(index_file_name.c_str(), ios::binary | ios::out);
...
bool employeeDatabase::isPresent(int eid) {
    ...
    while (index_file.read((char *)&current, sizeof(index_pair))) {
        if (current.key == eid) {
            index_file.close();
            return current.position != -1; // mark as deleted
        }
    }
    index_file.close();
    return false;
...
void employeeDatabase::display_all() {
    fstream index_file(index_file_name.c_str(), ios::binary | ios::in);
    fstream data_file(data_file_name.c_str(), ios::binary | ios::in);
    index_pair current;
    employee emp;
